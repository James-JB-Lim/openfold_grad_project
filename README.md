...
### Overview
This project was developed as part of my graduation requirement at Hanyang University. It focuses on protein structure prediction using the OpenFold library, specifically targeting CASP15 protein data.

### Settings
To set up the project environment, use the environment.yml file. Additionally, consult resources/openfold_params/README.txt for instructions on configuring the OpenFold library parameters.

### Usage
The core of this project is implemented in a Jupyter Notebook (test_openfold.ipynb). This notebook utilizes the OpenFold library to predict protein structures and generates PDB file as output. We have prepared .pkl files for several target proteins from the CASP15 dataset for efficent data processing. They contain all essential data required by the model, such as MSA and templates. For visual comparison of the predicted structure with the ground truth, you can use PyMOL to superpose them. See below to get more information. 
